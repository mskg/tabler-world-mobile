...
import { BatchState, SyncOrAsyncIterable, TableState, TableStateElement, ThrottledTableConfiguration } from './types';
...
export abstract class BatchOperation<
    ...
    public next(): Promise<IteratorResult<[string, Element]>> {
    ...
    private async refillPending() {
        while (
            ...
            if (isIteratorResult(toProcess)) {
                this.sourceDone = toProcess.done as boolean;
                if (!this.sourceDone) {

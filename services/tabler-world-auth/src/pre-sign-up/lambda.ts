...
export const handler: PreSignUpTriggerHandler = async (event, context) => {
    ...
    await withClient(context, async (client) => {
        ...
        if (res.rowCount !== 1) {
            throw new Error('You need to be an active member and have a valid Family.WORLD membership. Please sign-in with your \'@xxxx.roundtable.world\' or \'@xxxx.ladiescircle.world\' e-mail address.');
        }
    });

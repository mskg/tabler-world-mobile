 ...
 * per-table basis.
 */
export class BatchGet extends BatchOperation<DocumentClient.AttributeMap> {
    ...
    constructor(
        ...
        items: SyncOrAsyncIterable<[string, DocumentClient.AttributeMap]>,
        {
            ConsistentRead,
            PerTableOptions = {},
        }: BatchGetOptions = {},
    ...
    protected async doBatchRequest() {
        const operationInput: DocumentClient.BatchGetItemInput = { RequestItems: {} };
        let batchSize = 0;

        while (this.toSend.length > 0) {
            const [tableName, item] = this.toSend.shift() as [string, DocumentClient.AttributeMap];
        ...
        const {
            Responses = {},
            UnprocessedKeys = {},
        } = await this.client.batchGet(operationInput).promise();
    ...
    protected getInitialTableState(tableName: string): TableState<DocumentClient.AttributeMap> {
        const {
